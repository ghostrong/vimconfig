...
"hi Comment cterm=NONE ctermbg=darkblue ctermfg=white guibg=NONE guifg=lightblue
set shiftwidth=4
set encoding=utf-8
...
" status line
set statusline=%<%f\ %h%m%r%=%k[%{(&fenc==\"\")?&enc:&fenc}%{(&bomb?\",BOM\":\"\")}]\ <%=style=%{CodingStyleIndent()}>\ %-14.(%l,%c%V%)\ %P
set laststatus=2
...
function! s:InsertHeaderGuard()
    ...
    exec 'norm O#ifndef ' . varname . "_"
    exec 'norm o#define ' . varname . "_"
    "exec 'norm o#pragma once'
    exec '$norm o#endif  // ' . varname . "_"
    exec '$norm o'
endfunction
...
"set fileencodings=cp936,ucs-bom,utf-8-bom,utf-8,gb18030,ucs,big5
